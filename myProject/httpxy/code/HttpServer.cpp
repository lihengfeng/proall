...
int HttpServer::getLine(int fd,char*buf,int size){  //获取recv中一行数据,并且当读到\r\n进行处理
	int n=0;
	int i=0;
	char c='\0';
...
void HttpServer::notFound(int client){
	char buf[1024];
...
void HttpServer::readRequest(int clientFd){  
	int n=0;
	char buf[1024];
	bool cgi=false;
	struct stat st;
...
void HttpServer::headers(int clientFd){
	char buf[1024]={0};
...
void HttpServer::cat(int clientFd,ifstream& fs){  //发送文件内容
	string str;
...
void HttpServer::cat2(int clientFd,int fd){
	char buf[1025]={0};
	int readLen=-1;
...
//以c++的fstream的形式读取数据
void HttpServer::returnFile(int clientFd,const string& path){
	int n;
	char buf[1024];
...
//以c的read系统调用的形式读取数据
void HttpServer::returnFile2(int clientFd,const string& path){
	int n;
	char buf[1024];
	while(n!=0&&strcmp("\n",buf))//因为返回静态文本，不需要http其余部分，所以清空read中所有数据
		n=getLine(clientFd,buf,1024);
...
void HttpServer::cannot_execute(int client)  //无法执行cgi，所以是服务器原因，返回500状态
{
	char buf[1024];
...
void HttpServer::exec(int clientFd,const string& method,const string& path,const string& params){
	int n=0;
	int len=1;
	int contentLength=-1; 
...
void HttpServer::listenPort(int port){  //设置监听端口
	myPort=port;
	if(myFd>0)  //如果myFd大于0,说明有accpetSocket正在监听，需要先关闭
		::close(myFd);
	int serverFd=-1; 
...
void HttpServer::start(){ 
	...
	while(true){
		...
		if(clientFd==-1)  //如果因为accept错误退出，需要调用pool.stop().保证其他成功的socket完成后退出
		{
			perror("accept");
			pool.stop();
			break;
		...
		//加入线程池，
		pool.run(std::bind(&HttpServer::readRequest,this,clientFd));
	}
	close(myFd);
	sleep(1);
}
